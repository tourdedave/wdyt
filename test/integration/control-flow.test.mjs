...
test("artifact command packages current wdyt runtime state with manifest", { timeout: 15_000 }, async () => {
  ...
  try {
    ...
    assert.match(artifactHelp, /wdyt artifact export \[--format zip\] \[--output <path>\]/);
...
test("summary page renders executive overview shell with reordered navigation", { timeout: 15_000 }, async () => {
  ...
  try {
    ...
    assert.match(page, /Unique Behaviors/);
...
test("summary page exposes print action and zip download", { timeout: 15_000 }, async () => {
  ...
  try {
    ...
    const summaryPage = await fetch(`${serverUrl}/review/summary`).then((response) => response.text());
    assert.match(summaryPage, /Print \/ Save as PDF/);
    assert.match(summaryPage, /id="summaryPrintAction"/);
    assert.match(summaryPage, /window\.print\(\)/);
